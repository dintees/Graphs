...
double circuit_length(std::vector<Point> points) {
    double length = 0.0;
    for (int i = 0; i < points.size() - 1; ++i) length += distance(points[i], points[i+1]);
...
std::vector<Point> simulated_annealing(std::vector<Point> points) {
    ...
    for (int i = 100; i >= 1; --i) {
        ...
        for (int it = 0; it < IT_MAX; ++it) {
            ...
            if (P_new_cost < P_cost) P = P_new;
            else {
                double r = (double) rand() / RAND_MAX;
                if (r < exp(-(P_new_cost - P_cost) / (T))) P = P_new;
                else std::swap(P_new[b], P_new[c]);
            }
        }
    }
